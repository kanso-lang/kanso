...
/* Everything past the bare digit loop: libc's strtoll and the two refusals.
   ...
   every call, for a path no call in either benchmark takes. */
static __attribute__((noinline, cold)) KValue k_b_to_int_slow(const char* data, long long len, const char* origin) {
    char* end = NULL;
    errno = 0;
