...
static inline void k_beat_rewind(KMark* m) {
    ...
    if (__builtin_expect(!(k_buf_dirty | m->reg_any), 1)) {
        /* Nothing was allocated since the mark: the same pointer means the
           same room left, so the two stores below would write back what is
           already there, and no string can lie above a mark nothing was
           allocated past, so the cursor has nothing to forget. A loop that
           only pushes into a list it owns takes this exit every iteration;
           escapebench's 1.55 million did. The pointer is asked before the
           block because it answers for both: blocks never overlap, so a
           pointer equal to the mark's lies in the mark's block, and a loop
           that allocated nothing no longer loads the chain's head to be
           told so. */
        if (__builtin_expect(k_arena == m->ptr, 1)) return;
    }
    if (__builtin_expect(!(k_buf_dirty | m->reg_any) && k_blocks == m->block, 1)) {
...
/* Each block is twice the one before it. That is what keeps the list short
   ...
   for the two mallocs. */
static __attribute__((noinline, cold, preserve_most)) KTenBlock* k_ten_block_open(long long d, KTenBlock* b, size_t n) {
...
static void* k_ten_alloc(size_t n) {
    ...
    if (!b || b->cap - b->used < n) b = k_ten_block_open(d, b, n);
...
/* Tenuring is sticky, and it is never unset within one evacuation. A promoted
   ...
   it — and the cheap way to guarantee that is to stop asking. Everything the
   walk reaches after the first lived-a-lap node is tenured too, which costs a
...
static inline __attribute__((always_inline)) void k_copy_short(char* d, const char* s, long long n) {
...
static long long k_eq_rec(KValue a, KValue b) {
    ...
    if (b.tag == K_THUNK) return k_eq_rec(a, k_force(b));
...
static __attribute__((noinline, cold, preserve_most)) KValue* k_buf_perm(long long cap) {
...
/* The beat whose rewind reclaims this header, or -1 for a header older than
   every live mark. */
static __attribute__((noinline, cold, preserve_most)) void k_permreg_add(const void* header, KValue** slot) {
...
KValue k_closure(KValue (K_CLOSCC *fn)(void*, KValue), long long arity, long long ncaps, KValue* caps) {
    ...
    if (ncaps <= 8) {
        ...
        k_copy_cold(env, caps, sizeof(KValue) * ncaps);
    }
...
/* A failing field is the rare arm of `entries`, and k_rec is a plain call
   that would cost the hot arm its frame. */
static __attribute__((noinline, cold, preserve_most)) KValue k_rec_cold(long long t, long long n, KValue* f) {
    return k_rec(t, n, f);
}
...
/* The front door to utf-8 validation, and it answers without one for every
   ...
   seven constant loads and two zeroed accumulators before the first block —
   where the ascii test above is two loads, so the test belongs at the call
   site and the pass belongs behind a call. It was inside the pass, which put
   both behind the same call: 83,092,800 instructions for 1,571,250 answers on
   jsonbench, 53 apiece for a mean run of seven ascii bytes. A run that is not
...
/* The outgrown buffer of an accumulator came from malloc, and its release
...
static KValue k_b_push_grow(KValue lv, KList* l, KValue item, int mutate) {
    ...
    if (l->len <= 4) {
        ...
        k_copy_cold(items, l->items, sizeof(KValue) * l->len);
    }
    items[l->len] = item;
...
/* A grow's malloc regime and the free of a malloced predecessor are its
   rare arms -- 1,080 and 990 of 176,697 grows on the run program -- and
   both were plain calls the hot arm kept a frame for. */
static __attribute__((noinline, cold, preserve_most)) KBuf* k_bytes_buf_malloc(long long cap) {
...
static __attribute__((noinline)) KValue k_b_append_grow(KValue acc, KBytes* a,
    ...
    if (a->len) k_copy_cold(data, a->data, (size_t)a->len);
    ...
    if (mutate) {
        /* Only a buffer made here, from malloc, carries a clear bit 0 over a
           ...
           released. */
        if (k_bytes_malloced(a)) k_bytes_buf_release(((KBuf*)a->data) - 1);
        a->len += n;
        a->data = data;
        a->cap = marked;
        return acc;
...
/* The scan behind `find2_below`, with the KValue convention taken off its
   inputs. FIVE KValues are ten register-sized arguments where the SysV ABI has
   six, so four of them arrive on the caller's stack and the callee loads and
   unpacks each before it can splat a byte — the largest spill of the five
...
/* The walk keeps a dozen values live and paid seven pushes at k_b_slice's
   door, on every slice of any shape; as its own function in tail position
   the ascii and list arms open with none. */
static __attribute__((noinline)) KValue k_b_slice_walk(KStr* s, long long from, long long to) {
    {
