...
/// The two-universe rule, advised: an arm may turn an err into a value only
/// when the reason type belongs to a different published package. This sees
/// the case that needs no plumbing to be certain about — an arm and a reason
/// type carrying the same qualifier are the same module, so the same
/// universe — and stays quiet about the rest, where a qualifier alone cannot
/// tell a foreign package from a local subdirectory. Test files are exempt:
/// a package inspecting the errs it raises is the author reading their own
/// internals, and the alternative is failure paths nothing can test.
pub fn license_advisories(program: &Program, returns: &[crate::infer::Set]) -> Vec<String> {
    let mut advisories = Vec::new();
    let mut seen = HashSet::new();
    for (i, decl) in program.fns.iter().enumerate() {
        ...
        let own = decl.name.rsplit_once('/').map(|(q, _)| q);
        let Some(reason) = decl.params.iter().find_map(err_reason_type) else { continue };
        if reason.rsplit_once('/').map(|(q, _)| q) != own {
            continue;
        }
        ...
        if seen.insert((decl.name.clone(), reason.to_string())) {
            advisories.push(format!(
                "advisory[license]: `{}` turns its own `{reason}` into a value — \
                 an err is handled by the package that did not raise it; \
                 re-raise, or let callers name the reason themselves",
                decl.name
            ));
        }
    }
...
/// The reason type an err pattern names, if it names one.
fn err_reason_type(pattern: &Pattern) -> Option<&str> {
    let Pattern::Ctor { ty, fields } = pattern else { return None };
    if ty != "err" || fields.len() != 1 {
        return None;
    }
    match &fields[0] {
        Pattern::Annotated { ty, .. } => Some(ty.as_str()),
        Pattern::Ctor { ty, .. } => Some(ty.as_str()),
        _ => None,
    }
}
