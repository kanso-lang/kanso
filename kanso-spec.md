...
- **zero-field types (gaveled 2026-07-12):** `type null` with no field lines is legal. the bare mention of the name *is* its value; in pattern position the bare name matches that marker — unambiguous because no-shadowing already forbids a binding named after a declared type. a constructor call passing a zero-field type arguments is a compile error. markers behave like `true`/`false`/`none` mechanically but are ordinary data — no propagation. `null` is the conventional data-absence marker, declared in a library, never a keyword.
- **typeset fields (gaveled 2026-07-12):** a field may list several permitted types, space-separated, alphabetized like every enumeration: `title: null string`. members are deduplicated and each names a declared type or a builtin type word (`bool`/`float64`/`int`/`string`). constructors enforce membership — a field value matching no member is an endpoint-reported defect. reads need no new machinery: values dispatch as what they are, so absence is a polymorphic call, never a conditional. the absence member is a marker type, never `none`. several markers in one field are enums for free (`status: active archived pending`). v1 scope: fields only; parameter typesets wait for the annotation-redundancy checker.
