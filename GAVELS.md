...
| BB `return X if C` | ✅ "we do want return even if it technically cascades... the feeling has to be that you don't execute further code" — and it IS just returning: the parser folds everything below a guard into the untaken branch of a compiler-built conditional. Flat to read, nested in the tree, first-return-wins by unreachability. Returns live with the bindings; unconditional `return` is a teaching error (that's the final expression); one top-level `if` per return (parenthesize inner ones) | **implemented** |
