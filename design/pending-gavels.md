...
The compile-instructions bimodality that sat here on 2026-09-03 is
WITHDRAWN, and by measurement rather than by a ruling. It asked how to
live with a row reading two values 508 apart and offered four ways; the
first of them, "find the term", was a measurement, so it was taken. The
row was counting glibc's startup parse of `/proc/self/maps`, whose cost
belongs to the host's memory map and not to the compiler — global rather
than per-chip, which is what the six readings had already shown without
naming it. design/compiler-log.md, 2026-09-03 (ninth), carries the
evidence and the knobs that move it. What remains is repairing the
instrument, which is the implementer's and not yours.
