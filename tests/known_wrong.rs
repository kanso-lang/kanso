...
/// second's value. Five KValues want ten argument registers and AArch64 has
/// eight, so two travel on the stack and the dev build's `tailcc` call gets
/// that wrong. Silent corruption, not a crash — the interpreter and the
/// -flto release build both answer correctly.
#[test]
fn five_parameters_corrupt_the_second_call_on_the_dev_build() {
    ...
    assert_eq!(native_out, "[2 1 3 4 5][2 2 3 4 5]\n", "native no longer corrupts — delete this");
}
