...
# the answer when no declaration in the whole program carries the description
# bit -- neither the table nor the tail walk behind it can answer yes -- so the
# pass reads `any_boxed` once and skips all of it, leaving the chain, which the
...
set -e
grep -q '^    let any_boxed = returns.values().any(|(s, _, _)| s & DESC != 0);$' src/check.rs || {
