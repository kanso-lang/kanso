...
# same `> 0` test every other counting site makes. This mutation puts the
# two-branch gate back. The bytes out are the same and no counter moves --
...
sed -i '/^       allocations a run on the run program, one instruction each\. \*\/$/{n;s|^    if (__builtin_expect(k_stats_on > 0, 0)) {$|    if (__builtin_expect(k_stats_on != 0, 0)) if (k_stats_on) {|}' src/runtime.c
grep -qF '    if (__builtin_expect(k_stats_on != 0, 0)) if (k_stats_on) {' src/runtime.c
