...
# is UNBUILT rather than red. The walk spells its bound out in the `if`, so
# the plain read is proven in range and `text/append` takes the element
# (ruled 2026-09-16); a bang there would answer a box the append refuses.
set -e
...
sed -i 's|^  if (p < n) (text/append acc (text/slice bs p (n - 1))) acc$|  return acc if p < 1 or length bs < p or n <= p\n  escape_run (text/append acc bs[p]) bs (p + 1) n|' \
  lib/json/text.kso
grep -q '^  escape_run (text/append acc bs\[p\]) bs (p + 1) n$' lib/json/text.kso
