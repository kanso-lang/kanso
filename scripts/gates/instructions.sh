...
# THROWAWAY PROBE — not for merge. callgrind_annotate cannot render --dump-instr
# data (that needs KCachegrind) and --auto has no source to point at, because the
# emitted IR carries no debug metadata and no clang flag invents it. Call counts
# need neither: they say whether 642M is many cheap calls or few expensive ones.
echo "=== call counts into value_for"
callgrind_annotate --tree=calling --threshold=30 /tmp/cg.jsonbench 2>&1 |
  grep -B6 -A6 'value_for_3' | head -40
